...
public static class Program
{
    ...
    private static async Task RunInteractive()
    {
        ...
        while (true)
        {
            ...
            {
                if ("quit".StartsWith(input.ToLower())) break;
    ...
    private static async Task RunOperation(string[] args)
    {
        try
        {
            ...
            {
                await _generator.DeleteDatabase(true);
            }
    ...
    {
        return match.StartsWith(operation);
    }
    ...
    private static async Task ShowFlights()
    {
        ...
        foreach (var flight in flights)
        {
            ctr++;
            Console.WriteLine(
                $"[{ctr,2}] {flight.FlightNumber,-8}{flight.DepartureAirport} > {flight.ArrivalAirport}  {flight.DistanceMiles} miles");
        }
    }
