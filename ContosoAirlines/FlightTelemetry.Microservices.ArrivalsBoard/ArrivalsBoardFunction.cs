...
public static class ArrivalsBoardFunction
{
    ...
    private static readonly Dictionary<string, AirportArrivals> _board = new();
        ...
        ILogger logger)
    {
        ...
        if (ShouldSkip(includesCompletedFlights)) return;
    ...
    private static bool ShouldSkip(bool includesCompletedFlights)
    {
        ...
        if (includesCompletedFlights) return false;
        ...
        lock (_threadLock)
        {
            ...
            if (sinceLastUpdate.TotalSeconds < 5) return true;
