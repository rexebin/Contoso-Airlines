...
{
    private static readonly FlightSpatial _spatial = new();
    ...
    private static async Task CheckFlightLocation(Document document, ILogger logger)
    {
        ...
        if (!_spatial.IsInNoFlyZone(locationEvent.Latitude, locationEvent.Longitude)) return;
        lock (_threadLock)
        {
            if (_timestamps.TryGetValue(locationEvent.FlightNumber, out var lastEmailSent))
