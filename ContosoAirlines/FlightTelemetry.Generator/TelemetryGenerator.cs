...
public class TelemetryGenerator
{
    ...
    private readonly Airport[] _airports = new[]
    {
        new Airport
        {
            Code = "JFK", Longitude = -73.7781, Latitude = 40.6413, Name = "New York, NY: John F. Kennedy International"
        },
        new Airport
        {
            Code = "LAX", Longitude = -118.4085, Latitude = 33.9416, Name = "Los Angeles, CA: Los Angeles International"
        },
        new Airport
        {
            Code = "SEA", Longitude = -122.2961, Latitude = 47.4436, Name = "Seattle, WA: Seattle/Tacoma International"
        },
        new Airport
        {
            Code = "MCO", Longitude = -81.3081, Latitude = 28.4312, Name = "Orlando, FL: Orlando International"
        },
        new Airport
        {
            Code = "ORD", Longitude = -87.9073, Latitude = 41.9742, Name = "Chicago, IL: Chicago O'Hare International"
        },
        new Airport
        {
            Code = "DEN", Longitude = -104.6737, Latitude = 39.8561, Name = "Denver, CO: Denver International"
        }
    };

    private static readonly IDictionary<string, SpeedAltitudeEntry> _speedAndAltitude =
        new Dictionary<string, SpeedAltitudeEntry>();
    ...
    private readonly FlightRepo _flightRepo = new();
    ...
    private (string ContainerName, string PartitionKey, int Throughput, bool AutoScale, int? TimeToLive)[] Containers =>
        new (string, string, int, bool, int?)[]
        {
            (Constants.LeaseContainerName, "/id", 400, false, null), // for CFP Library
            (Constants.MetadataContainerName, "/type", 400, false,
                null), // for small lookup lists; pk=airport/flight; id=airport ID/flight ID (e.g., airport, flight)
            (Constants.LocationContainerName, "/id", 1000, true, null), // for ingestion; pk/id = GUID
            (Constants.CurrentLocationContainerName, "/type", 400, false,
                null), // for currentLocation microservice; pk=location; id=flight ID
            (Constants.ArrivalsBoardContainerName, "/type", 400, false,
                null) // for arrivalsBoard microservice; pk=arrival; id=arrival airport ID
        };
    ...
    {
        if (!Confirm(
                $"This will create the {Constants.DatabaseName} database. It will be deleted if it already exists."))
            return;
    ...
    protected async Task CreateContainer(string containerName, string partitionKey, int throughput, bool autoScale,
        int? timeToLive)
    {
        ...
        if (autoScale)
        {
            ...
            Console.WriteLine(
                $"Creating container '{containerName}', partitionKey = {partitionKey}, throughput = {throughput} - {throughput * 10} RUs");
        }
        else
        {
            throughputProperties = ThroughputProperties.CreateManualThroughput(throughput);
            Console.WriteLine(
                $"Creating container '{containerName}', partitionKey = {partitionKey}, throughput = {throughput} RUs");
        }
        ...
        await Cosmos.Client.GetDatabase(Constants.DatabaseName)
            .CreateContainerAsync(containerProperties, throughputProperties);
    }

    public async Task DeleteDatabase(bool confirm)
    {
        if (confirm && !Confirm($"This will delete the {Constants.DatabaseName} database, if it already exists."))
            return;
        ...
        if (deleted) Console.WriteLine($"Deleted database {Constants.DatabaseName}");
    }
    ...
    private async Task CreateAirports()
    {
        ...
        Console.WriteLine($"Created {_airports.Length} airports");
    }
    ...
    private async Task CreateFlights()
    {
        ...
        {
            var departureAirport = _airports.First(a => a.Code == flight.DepartureAirport);
            var arrivalAirport = _airports.First(a => a.Code == flight.ArrivalAirport);
            var distance = _spatial.CalculateDistance(departureAirport.Latitude, departureAirport.Longitude,
            ...
            flight.Type = "flight";
            flight.TailNumber = _random.Number(2, 9) * 97 + _random.AlphaNumeric(2) + _random.Number(1, 10);
        ...
        Console.WriteLine($"Created {_flights.Length} flights");
    }

    public async Task ResetFlights()
    {
        var airports = await _flightRepo.GetAirports();
        var flights = await _flightRepo.GetFlights();
        ...
        foreach (var flight in flights)
        {
            var result = await _flightRepo.QueryLocation(flight.FlightNumber);
    ...
    {
        Console.Write(
            $"Press any key to start generating telemetry {(continuous ? "continuously" : "through flight completion")}, or ESC to cancel... ");
        ...
        Console.WriteLine();
        Console.WriteLine(
            $"Flight  From > To  Arrives  Distance  Duration  Remaining           Speed    Altitude   Location");
        Console.WriteLine(
            $"------- ---------- -------  --------  --------  ------------------  -------- ---------  -------------------");

        _flights = await _flightRepo.GetFlights();

        var currentLine = Console.CursorTop;
        Console.SetCursorPosition(0, 5 + _flights.Length);
        ...
        _noFlyZoneCount = 0;
        _errorCount = 0;
        _completedFlights = new List<string>();
        ...
        while (true)
        {
            while (!Console.KeyAvailable)
            {
                ...
                var telemetry = GenerateTelemetry(continuous);
                ...
                {
                    var errorCount = await WriteTelemetry(container, telemetry);
                    var rate = Math.Round(_telemetryCount / elapsed.TotalSeconds, 0);
...
//						Console.WriteLine($"Flights={this._flights.Length}; Telemetry={this._telemetryCount} ({rate}/sec); NoFlyZone={this._noFlyZoneCount}; Errors={this._errorCount}; Elapsed={elapsed}");
                    Console.WriteLine(
                        $"Flights={_flights.Length}; Count={_telemetryCount} ({rate}/sec); Errors={_errorCount}; Elapsed={elapsed}");
                    await Task.Delay(PayloadIntervalMs);
                }
            }

            if (allFlightsComplete) break;
        ...
        Console.SetCursorPosition(0, 5 + _flights.Length);
        Console.WriteLine(
            $"Stopped after generating {_telemetryCount} telemetry items for {_flights.Length} flights with {_errorCount} errors");
    }
    ...
    private List<LocationEvent> GenerateTelemetry(bool continuous)
    {
        ...
        foreach (var flight in _flights)
        {
            if (_completedFlights.Contains(flight.FlightNumber))
            {
                Console.WriteLine();
                continue;
            }
            ...
            ctr++;
            var arrivalAirport = _airports.First(a => a.Code == flight.ArrivalAirport);
            var remainingMiles = (int)_spatial.CalculateDistance(flight.Latitude, flight.Longitude,
                arrivalAirport.Latitude, arrivalAirport.Longitude);
            var remainingMinutes = remainingMiles / 500.0 * 60; // assume 500 mph

            var speedAndAltitude = GenerateSpeedAndAltitude(flight.FlightNumber);
            ...
            Console.WriteLine(
                $"{item.FlightNumber,6}  {item.DepartureAirport} > {item.ArrivalAirport}    {flight.ArrivalTime:HH:mm}   {flight.DistanceMiles,4} mi   {Math.Round(flight.DurationMinutes / 60, 2),-4} hr    {remainingMiles,4} mi, {Math.Round(remainingMinutes / 60, 2),-4:0.00} hr   {item.Speed} mph  {item.Altitude} ft {item.Latitude,9:##00.0000}, {item.Longitude,9:##00.0000}   ");
            list.Add(item);
            _telemetryCount++;

            if (item.IsComplete)
            {
                _completedFlights.Add(flight.FlightNumber);
    ...
    private SpeedAltitudeEntry GenerateSpeedAndAltitude(string flightNumber)
    {
        lock (_threadLock)
        {
            if (_speedAndAltitude.TryGetValue(flightNumber, out var speedAndAltitude))
            ...
            {
                Speed = _random.Number(490, 510),
                Altitude = _random.Number(37000, 38000),
    ...
    private void NudgeFlight(Flight flight, bool continuous)
    {
        while (true)
        {
            var subtractLat = _random.Number(0, 1) == 0;
            var subtractLon = _random.Number(0, 1) == 0;
            var valueLat = _random.Number(50, 60) / 1500.0;
            var valueLon = _random.Number(50, 60) / 1500.0;
            ...
            var arrivalAirport = _airports.First(a => a.Code == flight.ArrivalAirport);
            var prevRemainingDistance =
                _spatial.CalculateDistance(lat, lon, arrivalAirport.Latitude, arrivalAirport.Longitude);
            ...
            var newRemainingDistance = _spatial.CalculateDistance(flight.Latitude, flight.Longitude,
                arrivalAirport.Latitude, arrivalAirport.Longitude);
            ...
            if (prevRemainingDistance < newRemainingDistance)
            {
                ...
                if (_spatial.IsInNoFlyZone(lat, lon))
                {
                    _noFlyZoneCount++;
                    break;
                }
    ...
    protected async Task<int> WriteTelemetry(Container container, List<LocationEvent> docs)
    {
        ...
        foreach (var item in docs)
        {
            tasks[taskCtr] = container
                ...
                .ContinueWith(task => errorCtr = CatchInsertDocumentError(task, errorCtr));
    ...
    private int CatchInsertDocumentError(Task<ItemResponse<LocationEvent>> task, int errorCtr)
    {
        if (!task.IsCompletedSuccessfully)
        {
            ...
            {
                var ctr = 0;
                foreach (var ex in ae.Flatten().InnerExceptions) Console.WriteLine($"[{++ctr}] {ex.Message}");
            }
            else
            {
    ...
    private bool Confirm(string message)
    {
        ...
        while (true)
        {
            ...
            if (input.ToLower().StartsWith("n")) return false;
        }
    }
