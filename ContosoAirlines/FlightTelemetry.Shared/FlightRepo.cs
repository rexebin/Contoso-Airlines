...
namespace FlightTelemetry.Shared
{
    public class FlightRepo
    {
        ...
        {
            return await GetList<Airport>("airport");
        }

        public async Task<Flight[]> GetFlights()
        {
            return await GetList<Flight>("flight");
        }
        ...
        public async Task<(LocationEvent LocationEvent, double Cost)> ReadCurrentLocation(string flightNumber)
        {
            ...
            {
                var container =
                    Cosmos.Client.GetContainer(Constants.DatabaseName, Constants.CurrentLocationContainerName);
        ...
        public async Task<(LocationEvent[] LocationEvents, string Sql, double Cost)> QueryCurrentLocations(
        ...
        public async Task<(AirportArrivals AirportArrivals, double Cost)> ReadAirportArrivals(string airportCode)
        {
            ...
            {
                var container =
                    Cosmos.Client.GetContainer(Constants.DatabaseName, Constants.ArrivalsBoardContainerName);
